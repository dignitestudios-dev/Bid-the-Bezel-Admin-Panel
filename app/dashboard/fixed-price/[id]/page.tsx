...
const Page = () => {
  ...
  const { data, isLoading } = useGetFixedPriceById(id);
  ...
  if (isLoading) {
    return (
      <div className="p-6 space-y-6">
        <Skeleton className="h-87.5 bg-gray-300 w-full rounded-xl" />
        <Skeleton className="h-10 bg-gray-300 w-1/3" />
        <Skeleton className="h-24 bg-gray-300 w-full" />
      </div>
    );
  }
  ...
  const confirmDelete = () => {
    ...
    deleteProduct(
      ...
      {
        onSuccess: () => {
          router.push("/dashboard/fixed-price");
          setOpen(false);
          setSelectedId(null);
        },
      },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl border bg-background p-6 space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <Badge className="bg-green-100 text-green-700 border-0">Sold</Badge>
          </div>
